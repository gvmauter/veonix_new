...
abstract class CWDI_Excel_All extends CWDI_Handler {
	...
	public function IncludePhpExcel(){
		...
		if(!class_exists('CWDI_ChunkReadFilter')){
			require_once(dirname(__FILE__).'/CWDI_Excel_ChunkReadFilter.php');
		}
	}
	 ...
	 *	Создание рендерера
	 */
	public function CreateRender($StartRow,$ChunkSize){
		...
		if(is_object($this->objExcelReader)){
			// Set filter
			$objFilter = new CWDI_ChunkReadFilter();
			$objFilter->SetRows($StartRow,$ChunkSize);
	...
	public static function GettingFile($arFields, $arHandler, $DownloadCallback=false){
		...
		switch($arParams['HANDLER_FILE_TYPE']){
			...
			case 'download':
				...
				break;
			case 'ftp':
				$FileName = self::DownloadFileFTP($arFields,$arHandler,false,$DownloadCallback);
				break;
			case 'search':
	...
	public static function DownloadFile($FileUrl, $arFields, $arHandler, $DownloadCallback=false){
		if(is_dir($_SERVER['DOCUMENT_ROOT'].$arHandler['TMP_DIR'])) {
			...
			if(!empty($BaseName)) {
				...
				if (!empty($strData)) {
					$bSaved = file_put_contents($_SERVER['DOCUMENT_ROOT'].$FileName,$strData);
					unset($arRequestParams,$strData);
					if($bSaved){
						return $FileName;
					}
				}
	 ...
	 */
	public static function DownloadFileFTP($arFields, $arHandler, $DownloadCallback=false){
		if(is_dir($_SERVER['DOCUMENT_ROOT'].$arHandler['TMP_DIR'])) {
			$arRequestParams = array(
				'FTP' => true,
				'HOST' => $arFields['PARAMS']['FTP_HOST'],
				'PORT' => $arFields['PARAMS']['FTP_PORT'],
				'LOGIN' => $arFields['PARAMS']['FTP_LOGIN'],
				'PASSWORD' => $arFields['PARAMS']['FTP_PASSWORD'],
				'SECURE' => $arFields['PARAMS']['FTP_SECURE'],
				'PASSIVE' => $arFields['PARAMS']['FTP_PASSIVE'],
				'FILENAME' => $arFields['PARAMS']['FTP_FILENAME'],
			);
			$BaseName = basename($arRequestParams['FILENAME']);
			if(!empty($BaseName)) {
				$BaseName = CWDI_Handler::GetFileBaseName($BaseName, $arFields['ID']);
				$Dir = $arHandler['TMP_DIR'];
	...
	public function OpenFile(array $arParams){
		if(is_file($this->arFile['FILE_ABS'])) {
			$Time = CWDI::GetMicroTime();
			$Mem = memory_get_usage();
			$this->CreateRender($arParams['START_ROW'],$arParams['CHUNK_SIZE']);
			try {
			...
			$Time = CWDI::GetMicroTime() - $Time;
			$Mem = memory_get_usage() - $Mem;
			$GLOBALS['WDI_EXCEL_FILE_LOADED_TIME'] = $Time;
			$GLOBALS['WDI_EXCEL_FILE_LOADED_MEMORY'] = $Mem;
			if (is_object($this->objPHPExcel)) {
				return true;
			}
		}
	...
	public function IsRowEmpty($RowIndex) {
		if($RowIndex>0) {
			foreach ($this->objSheet->GetRowIterator($RowIndex)->current()->GetCellIterator() as $Key => $Cell) {
				if ($Cell->GetValue()) {
					return false;
				}
			}
	...
	public function GetSheetsList(){
		...
		if(is_file($this->arFile['FILE_ABS']) && class_exists('XMLReader')) {
			$this->CreateRender($StartRow=1,$ChunkSize=1);
	...
	function GetRichTextValue($Row, $Col) {
		...
		if(is_object($Value) && $Value instanceof PHPExcel_RichText) {
			...
			unset($arValueElemens,$Class,$arItem);
		}
		return $arResult;
	}
	...
	function GetHtmlFromElements($arElements, $arHtmlFormatting=false) {
		...
		foreach($arElements as $arHTMLElement) {
			...
			unset($arTags,$arStyles,$strHTMLElement);
		}
		return $strResult;
	}
	...
	public function ProcessMatchSingle($Target, $arMatch, $arObject, $arParams){ // ToDo: abstract
		...
		if(preg_match('#^COLUMN_(\d+)$#i',$arMatch['SOURCE'],$M)){
			...
			$arParams['SEARCH_IMAGES_RECURSIVE'] = $this->arFields['PARAMS']['S'][$this->intSheetIndex]['SEARCH_IMAGES_RECURSIVE'];
			$Value = array(
				'RAW' => $Value,
				'MATCH' => $arMatch,
				'PARAMS' => $arParams,
