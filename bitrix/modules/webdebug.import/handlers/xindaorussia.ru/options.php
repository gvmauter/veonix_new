...
if($_GET['action']=='show_handler_settings' && $_GET['subaction']=='check_connection'){
	...
	print CWDI_XindaoRussia::CheckConnection() ? 'Y' : 'N';
	die();
}
?>
...
<?if($arData['MATCHES_ONLY']!='Y'):?>
	<table class="adm-detail-content-table">
		<tbody>
			<tr>
				<td class="adm-detail-content-cell-l" width="40%" valign="middle"><?=WdiHint(GetMessage('WDI_XINDAORUSSIA_CONNECTION_HINT'));?> <?=GetMessage('WDI_XINDAORUSSIA_CONNECTION');?></td>
				<td class="adm-detail-content-cell-r" width="60%">
					<div id="handler_file_type">
						<input type="button" value="<?=GetMessage('WDI_XINDAORUSSIA_CHECK_CONNECTION');?>" id="wdi_xindaorussia_check_connection" />
						<span id="wdi_xindaorussia_connection_status"></span>
						<span id="wdi_xindaorussia_connection_loader" style="display:none;"><?=WDI_LOADER;?></span>
						<script>
						$('#wdi_xindaorussia_check_connection').click(function(E){
							E.preventDefault();
							$('#wdi_xindaorussia_connection_loader').show();
							$.ajax({
								...
								success: function(HTML) {
									if(HTML=='Y') {
										$('#wdi_xindaorussia_connection_status').addClass('success').removeClass('error').attr('title','<?=GetMessage('WDI_XINDAORUSSIA_CONNECTION_SUCCESS');?>');
									} else {
										$('#wdi_xindaorussia_connection_status').addClass('error').removeClass('success').attr('title','<?=GetMessage('WDI_XINDAORUSSIA_CONNECTION_ERROR');?>');
									}
									$('#wdi_xindaorussia_connection_loader').hide();
								},
								error: function(){
									$('#wdi_xindaorussia_connection_status').addClass('error').removeClass('success');
									$('#wdi_xindaorussia_connection_loader').hide();
								}
							});
						});
...
$objXindaoRussia = new CWDI_XindaoRussia($arFields,$arHandler);
$arData['MATCHES'] = $objXindaoRussia->MakeMatches($arData['SHEET_INDEX']);
?>
...
<div id="wdi_handler_matches">
	...
	<table class="adm-detail-content-table">
		<tbody>
			<tr class="heading"><td colspan="2"><?=GetMessage('WDI_XINDAORUSSIA_PARAMETERS_MAIN');?></td></tr>
