...
class TurboFeed
{
	...
	{
		$arFilter = array();
		$arFilter = array_merge($arFilter, (array)$this->feed['OFFERS_FILTER']);
    ...
    protected function prepareItem($arItem = array())
    {
		...
		$arItems['LINK'] = str_replace('#YEAR#', $year[2], $arItems['LINK']);
	...
	public function prepareTurboContent($turboContent) 
	{
		...
		$turboContent = preg_replace('/<h1[^>]*?>/', '<h1>', $turboContent);
		$turboContent = preg_replace('/<h2[^>]*?>/', '<h2>', $turboContent);
		$turboContent = preg_replace('/<h3[^>]*?>/', '<h3>', $turboContent);
	...
	{
		return '<p><a><br><h1><h2><h3><h4><h5><h6><figure><img><figcaption><header><ul><ol><li><table><th><tr><td><video><source><b><strong><i><em><sup><sub><ins><del><small><big><pre><iframe><blockquote>';
	}
	...
	protected function getDirTemplates($templates = '.default')
	{
		...
		if(file_exists($pathToAction = $_SERVER['DOCUMENT_ROOT'].'/bitrix/php_interface/include/yandex_turbo/'.$templates))
		{
		...
		{
			$pathToAction = $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/goodde.yandexturboapi/load/templates/'.$templates;
		}
		...
		if($directory->isExists())
		{
			...
			foreach($arChildren as $k => $file)
			{
				if($file->isExists())
				{
					...
					$result = \Goodde\YandexTurbo\Model\Request::addFeed($arFields['LID'], 'production', $data, $this->isGzip);
					...
					{
						if($result['error_code'] != 'TOO_MANY_REQUESTS_ERROR')
						{
							$file->delete();
						}
	...
	public function addArchive($path = '', $data = '')
	{
		...
		if($file->isExists())
		{
			...
			else
			{
				...
				$put = file_put_contents($fileNameZip, gzencode($data, 9));
			}
			if(intval($put) > 0)
			{
				$file->delete();
