...
if ($RIGHT_R)
{
	...
	if((!empty($save) || !empty($restoreDefaults)) && $request->isPost() && $RIGHT_W && check_bitrix_sessid()) 
	{
	...
	if($request->isPost() && $RIGHT_W && check_bitrix_sessid())
	{
		if(!empty($getUserId)) 
		{
			foreach($siteList as $val)
			{
				...
				Option::set(
					$module_id,
					"turbo_prod",
					serialize(array_merge($arTurboProp, Request::curUser($val["ID"]))),
					$val['ID']
				);
			}
	...
	<form method="post" action="<?=sprintf('%s?mid=%s&lang=%s', $request->getRequestedPage(), urlencode($mid), LANGUAGE_ID)?>">
		...
		<?require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/admin/group_rights.php");?>
		<?$tabControl->Buttons();?>		
		<input <?if(!$RIGHT_W) echo "disabled" ?> type="submit" name="save" class="adm-btn-save" value="<?=Loc::getMessage("MAIN_SAVE")?>" title="<?=Loc::getMessage("MAIN_OPT_SAVE_TITLE")?>">
