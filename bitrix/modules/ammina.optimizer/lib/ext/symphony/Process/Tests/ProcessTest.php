...
class ProcessTest extends TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$sigchild = false !== amopt_strpos(ob_get_clean(), '--enable-sigchild');
    }
    ...
    public function testStopWithTimeoutIsActuallyWorking()
    {
        ...
        while ($p->isRunning() && false === amopt_strpos($p->getOutput(), 'received')) {
            usleep(1000);
        }
    ...
    public function testWaitUntilSpecificOutput()
    {
        ...
        $result = $p->waitUntil(function ($type, $output) use (&$completeOutput) {
            return false !== amopt_strpos($completeOutput .= $output, 'One more');
        });
    ...
    public function testAllOutputIsActuallyReadOnTermination()
    {
        ...
        $this->assertEquals($expectedOutputSize, amopt_strlen($o));
    }
    ...
    public function testProcessPipes($code, $size)
    {
        ...
        $this->assertEquals($expectedLength, amopt_strlen($p->getOutput()));
        $this->assertEquals($expectedLength, amopt_strlen($p->getErrorOutput()));
    }
    ...
    public function testIncrementalOutput($getOutput, $getIncrementalOutput, $uri)
    {
        ...
        foreach (['foo', 'bar'] as $s) {
            while (false === amopt_strpos($p->$getOutput(), $s)) {
                usleep(1000);
            }
    ...
    public function testUpdateStatus()
    {
        $process = $this->getProcess('echo foo');
        $process->run();
        $this->assertGreaterThan(0, amopt_strlen($process->getOutput()));
    }
    ...
    public function testIdleTimeoutNotExceededWhenOutputIsSent()
    {
        ...
        while (false === amopt_strpos($process->getOutput(), 'foo')) {
            usleep(1000);
        }
    ...
    public function testSignal()
    {
        ...
        while (false === amopt_strpos($process->getOutput(), 'Caught')) {
            usleep(1000);
        }
