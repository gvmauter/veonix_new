...
class Process implements \IteratorAggregate
{
	 ...
	 */
	public function __construct($command, string $cwd = NULL, array $env = NULL, $input = NULL, float $timeout = NULL)
	{
		if (is_null($timeout)) {
			$timeout = 60;
		}
	 ...
	 * @throws RuntimeException When proc_open is not installed
	 */
	public static function fromShellCommandline(string $command, string $cwd = NULL, array $env = NULL, $input = NULL, float $timeout = NULL)
	{
		if (is_null($timeout)) {
			$timeout = 60;
		}
	...
	protected function isSigchildEnabled()
	{
		...
		return self::$sigchild = false !== amopt_strpos(ob_get_clean(), '--enable-sigchild');
	}
	 ...
	 */
	private function readPipesForOutput(string $caller, bool $blocking = NULL)
	{
		if (is_null($blocking)) {
			$timeout = false;
		}
	 ...
	 */
	private function validateTimeout(float $timeout)
	{
	 ...
	 * @throws RuntimeException In case of failure
	 */
	private function doSignal(int $signal, bool $throwException)
	{
	...
	private function prepareWindowsCommandLine(string $cmd, array &$env)
	{
		...
		$cmd = preg_replace_callback(
			...
			function ($m) use (&$env, &$varCache, &$varCount, $uid) {
				...
				if (false !== amopt_strpos($value = $m[1], "\0")) {
	...
	private function escapeArgument(string $argument)
	{
		...
		if (false !== amopt_strpos($argument, "\0")) {
