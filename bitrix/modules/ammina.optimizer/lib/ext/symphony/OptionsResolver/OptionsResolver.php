...
class OptionsResolver implements Options
{
	...
	public function isNested(string $option)
	{
	 ...
	 */
	public function setDeprecated(string $option, $deprecationMessage = 'The option "%name%" is deprecated.')
	{
		if ($this->locked) {
	...
	public function isDeprecated(string $option)
	{
	...
	public function offsetGet($option/*, bool $triggerDeprecation = true*/)
	{
		...
		if (isset($this->allowedTypes[$option])) {
			...
			foreach ($this->allowedTypes[$option] as $type) {
				$type = self::$typeAliases[$type] ? $type : NULL;
			...
			if (!$valid) {
				...
				if (1 === \count($keys) && '[]' === amopt_substr($keys[0], -2)) {
	...
	private function verifyTypes(string $type, $value, array &$invalidTypes, int $level = NULL)
	{
		if (is_null($level)) {
			$level = 0;
		}
		if (\is_array($value) && '[]' === amopt_substr($type, -2)) {
			$type = amopt_substr($type, 0, -2);
		...
		if (!$invalidTypes) {
			$suffix = '';
			while (amopt_strlen($suffix) < $level * 2) {
				$suffix .= '[]';
			}
