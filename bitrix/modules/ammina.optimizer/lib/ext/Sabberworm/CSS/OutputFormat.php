...
class OutputFormat {
	...
	*/
	...
	public function set($aNames, $mValue) {
		$aVarPrefixes = array('a', 's', 'm', 'b', 'f', 'o', 'c', 'i');
		if(is_string($aNames) && amopt_strpos($aNames, '*') !== false) {
	...
	public function __call($sMethodName, $aArguments) {
		if(amopt_strpos($sMethodName, 'set') === 0) {
			return $this->set(amopt_substr($sMethodName, 3), $aArguments[0]);
		} else if(amopt_strpos($sMethodName, 'get') === 0) {
			return $this->get(amopt_substr($sMethodName, 3));
