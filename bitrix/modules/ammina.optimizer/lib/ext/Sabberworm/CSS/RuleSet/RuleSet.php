...
abstract class RuleSet implements Renderable, Commentable {
			...
			if($oParserState->getSettings()->bLenientParsing) {
				try {
					...
					try {
						...
						if($oParserState->streql(amopt_substr($sConsume, -1), '}')) {
	...
	public function getRules($mRule = null) {
		...
		foreach($this->aRules as $sName => $aRules) {
			...
			if(!$mRule || $sName === $mRule || (amopt_strrpos($mRule, '-') === amopt_strlen($mRule) - amopt_strlen('-') && (amopt_strpos($sName, $mRule) === 0 || $sName === amopt_substr($mRule, 0, -1)))) {
	...
	public function removeRule($mRule) {
		if($mRule instanceof Rule) {
			...
			foreach($this->aRules as $sName => $aRules) {
				...
				if(!$mRule || $sName === $mRule || (amopt_strrpos($mRule, '-') === amopt_strlen($mRule) - strlen('-') && (amopt_strpos($sName, $mRule) === 0 || $sName === amopt_substr($mRule, 0, -1)))) {
