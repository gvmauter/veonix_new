...
if($GKS_RIGHT>="R" && Loader::includeModule($module_id)):
	...
	else 
	{
		...
		if(is_array($SETTINGS)) 
		{
			...
			if(count($arErrors)<=0) 
			{
				...
				// sorting
				$TABS = CGrain_CustomSettingsOptions::SortArrayByKey($TABS,"SORT","ASC",false,"INT");
				foreach($TABS as &$TAB)
					$TAB['FIELDS'] = CGrain_CustomSettingsOptions::SortArrayByKey($TAB['FIELDS'],"SORT","ASC",false,"INT");
				unset($TAB);
				// /sorting
...
$tabControl->Begin();
?>
<form method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&lang=<?php echo LANG?>" name="gcs_settings_form">
<?php
...
.gcustomsettings-settings-sort-caption { font-size: 9px !important; }
input.gcustomsettings-settings-sort-field { font-size: 9px !important; line-height: 9px !important; height: 17px !important; width: 35px !important; margin-top: 3px !important; margin-left: 2px !important; padding: 0 4px !important; vertical-align: baseline !important; }
...
<?php if(CGrain_CustomSettingsOptions::IsLinksInstalled()):?>
		...
		<tr>
			...
			<td valign="top" width="50%">
				<select name="SETTINGS[PARENT_MENU]">
					<option value="global_menu_content"<?php if($arCustomPage["PARENT_MENU"]=="global_menu_content"):?> selected="selected"<?php endif?>><?=GetMessage("GRAIN_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_content")?></option>
					<option value="global_menu_services"<?php if($arCustomPage["PARENT_MENU"]=="global_menu_services"):?> selected="selected"<?php endif?>><?=GetMessage("GRAIN_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_services")?></option>
					<option value="global_menu_store"<?php if($arCustomPage["PARENT_MENU"]=="global_menu_store"):?> selected="selected"<?php endif?>><?=GetMessage("GRAIN_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_store")?></option>
					<option value="global_menu_statistics"<?php if($arCustomPage["PARENT_MENU"]=="global_menu_statistics"):?> selected="selected"<?php endif?>><?=GetMessage("GRAIN_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_statistics")?></option>
					<option value="global_menu_settings"<?php if($arCustomPage["PARENT_MENU"]=="global_menu_settings"):?> selected="selected"<?php endif?>><?=GetMessage("GRAIN_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_settings")?></option>
				</select>
			</td>
		</tr>
		<tr>
...
<input type="submit" <?php if ($GKS_RIGHT<"W") echo "disabled" ?> name="Update" value="<?php echo GetMessage("MAIN_SAVE")?>">
<input type="hidden" name="Update" value="Y">
<input type="submit" <?php if ($GKS_RIGHT<"W") echo "disabled" ?> name="Apply" value="<?php echo GetMessage("MAIN_APPLY")?>">
<?php if(strlen($_REQUEST["back_url_settings"])>0):?>
	<input type="button" name="Cancel" value="<?=GetMessage("MAIN_OPT_CANCEL")?>" title="<?=GetMessage("MAIN_OPT_CANCEL_TITLE")?>" onclick="window.location='<?php echo htmlspecialchars(CUtil::addslashes($_REQUEST["back_url_settings"]))?>'">
	<input type="hidden" name="back_url_settings" value="<?=htmlspecialchars($_REQUEST["back_url_settings"])?>">
<?php endif?>
	<input type="submit" <?php if ($GKS_RIGHT<"W") echo "disabled" ?> name="RestoreDefaults" title="<?php echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS")?>" OnClick="return confirm('<?php echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>')" value="<?php echo GetMessage("MAIN_RESTORE_DEFAULTS")?>">
	<?=bitrix_sessid_post();?>
<?php $tabControl->End();?>
</form>
<?php
