...
if ($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid()) {
    ...
    if (isset($_REQUEST['ptb_canonical_edit_apply']) || isset($_REQUEST['ptb_canonical_edit_save'])) {
        ...
        if ($result->isSuccess()) {
            ...
            $backUrl = 'ptb_canonical_edit.php?ID=' . $result->getId() . '&lang=' . LANG . GetFilterParams("filter_", false);
            if ($_REQUEST['backurl'] && ! $_REQUEST['ptb_canonical_edit_apply']) {
                $backUrl = $_REQUEST['backurl'];
            } elseif ($_REQUEST['ptb_canonical_edit_save']) {
                $backUrl = 'ptb_canonical_list.php?lang=' . LANG . GetFilterParams("filter_", false);
            }
            
            if ($_REQUEST['backurl'] && $_REQUEST['ptb_canonical_edit_apply']) {
                $backUrl .= '&backurl=' . $_REQUEST['backurl'];
            }
...
if ($ID > 0) {
    ...
    $arItem['PAGE'] = trim($_REQUEST['URL']);
    $arItem['SITE_ID'] = trim($_REQUEST['SITE_ID']);
    
    if ($arItem['PAGE'] && $arItem['SITE_ID']) {
        $item = ListTable::getRow([
            'filter' => [
                '=PAGE' => $arItem['PAGE'],
        ...
        if (! is_null($item)) {
            $url = 'ptb_canonical_edit.php?ID=' . $item['ID'] . '&lang=' . LANG . GetFilterParams("filter_", false);
            if ($_REQUEST['backurl']) {
                $url .= '&backurl=' . $_REQUEST['backurl'];
            }
            LocalRedirect($url);
        }
    }
...
<?endif ?>
	<tr>
		...
		<td class="adm-detail-content-cell-r"><input type="checkbox" value="Y"
			...
			<?=($arItem['ACTIVE'] == 'Y' || $ID <= 0) ? ' checked' : ''?>
			name="PTBCANONICAL[ACTIVE]" /></td>
	</tr>
	...
	<tr>
		...
		<td class="adm-detail-content-cell-r"><input type="checkbox" value="Y"
			...
			<?=$arItem['USE_REGEXP'] == 'Y' ? ' checked' : ''?>
			name="PTBCANONICAL[USE_REGEXP]" />&nbsp;&nbsp;&nbsp;<span
			class="adm-favorites-cap-hint-text"><?=Loc::getMessage('PTB_CANONICAL_EDIT_REQEXP_EXAMPLE') ?></span></td>
	</tr>

	<tr>
		<td class="adm-detail-content-cell-l"><label for="ptb_canonical_sort"><?=Loc::getMessage("PTB_CANONICAL_EDIT_SORT");?></label>:</td>
		<td class="adm-detail-content-cell-r"><input type="text"
			value="<?=$arItem['SORT'] > 0 ? $arItem['SORT'] : 500?>"
			id="ptb_canonical_sort" name="PTBCANONICAL[SORT]" />&nbsp;&nbsp;&nbsp;<span
