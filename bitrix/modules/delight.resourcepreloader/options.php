...
$MODULE_ID = "delight.resourcepreloader";

$arAllOptions = array(
    array(
		"name" => "enabled",
		"title" =>GetMessage("DELIGHT_PRELOADER_SETTINGS_ENABLED"),
		"default_value" => "Y",
		"type" => array("checkbox", "Y"),
	),
    array(
		"name" => "only_head",
		"title" =>GetMessage("DELIGHT_PRELOADER_SETTINGS_ONLY_HEAD"),
		"default_value" => "N",
		"type" => array("checkbox", "Y"),
	),
);
$aTabs = array(
    array("DIV" => "edit_main", "TAB" => GetMessage("DELIGHT_PRELOADER_SETTINGS_MAIN_TAB_SET"), "TITLE" => GetMessage("DELIGHT_PRELOADER_SETTINGS_MAIN_TAB_SET_TITLE")),
);
...
$tabControl->Begin();
?>
<form method="post" name="delight_preloader_settings" id="delight_preloader_settings" action="<? echo $APPLICATION->GetCurPage() ?>?mid=<?= urlencode($mid) ?>&amp;lang=<? echo LANGUAGE_ID ?>">
    ...
    <? $tabControl->Buttons(); ?>
		<input type="submit" name="Update" value="<?= GetMessage("DELIGHT_PRELOADER_SETTINGS_SAVE") ?>" title="<?= GetMessage("DELIGHT_PRELOADER_SETTINGS_SAVE") ?>" class="adm-btn-save">
		<input type="submit" name="Apply" value="<?= GetMessage("DELIGHT_PRELOADER_SETTINGS_APPLY") ?>" title="<?= GetMessage("DELIGHT_PRELOADER_SETTINGS_APPLY") ?>">
		<? if (strlen($_REQUEST["back_url_settings"]) > 0): ?>
			<input type="button" name="Cancel" value="<?= GetMessage("DELIGHT_PRELOADER_SETTINGS_CANCEL") ?>" title="<?= GetMessage("DELIGHT_PRELOADER_SETTINGS_CANCEL") ?>" onclick="window.location = '<? echo htmlspecialcharsbx(CUtil::addslashes($_REQUEST["back_url_settings"])) ?>'">
			<input type="hidden" name="back_url_settings" value="<?= htmlspecialcharsbx($_REQUEST["back_url_settings"]) ?>">
		<? endif ?>
		<input type="submit" name="RestoreDefaults" title="<? echo GetMessage("DELIGHT_PRELOADER_RESTORE_DEFAULTS") ?>" OnClick="return confirm('<? echo AddSlashes(GetMessage("DELIGHT_PRELOADER_RESTORE_DEFAULTS_WARNING")) ?>')" value="<? echo GetMessage("DELIGHT_PRELOADER_RESTORE_DEFAULTS") ?>">
		<?= bitrix_sessid_post(); ?>
    <? $tabControl->End(); ?>
</form>
<script>
    BX.ready(function() {
        BX.UI.Hint.init(BX('delight_preloader_settings'));
		BX.ajax({
